...
prediction_columns <- c('predicted_RF', 'predicted_KNN', 'predicted_XGB')
...
Compared_RF_KNN_XGB <- subset(All_results, select = c('Cell_type','predicted_RF', 'predicted_KNN','predicted_XGB','all_predictions_same')) 

# Add a new column with the common value appearing at least twice
Compared_RF_KNN_XGB <- Compared_RF_KNN_XGB %>%
  rowwise() %>%
  mutate(common_value = {
    counts <- table(c(predicted_RF, predicted_KNN, predicted_XGB))
    common_values <- names(counts[counts >= 2])
    if (length(common_values) > 0) {
      common_values[1]
    } else {
      "Pippo" #Chose a column
    }
  })
...
cat("\nUpdated Data Frame:\n")
print(Compared_RF_KNN_XGB)

setwd("D:/VarieTHOM/University/QCB/3_SEMESTRE/Data Mining/Laboratory (Blanzieri)/0_PROJECT/Datasets_finals/ML_nonHS")
write.csv(Compared_RF_KNN_XGB, "Compared_RF_KNN_XGB.csv", row.names = TRUE)
...
plot_ly(
  ...
  layout(
    title = "Same Predictions on nonHS",
...
#pairs(All_results[, c('Cell_type','predicted_RF', 'predicted_KNN', 'predicted_XGB', 'predicted_NB')])
summary(All_results[, c('Cell_type','predicted_RF', 'predicted_KNN', 'predicted_XGB', 'predicted_NB')])
summary(Compared_RF_KNN_XGB[,c('predicted_RF', 'predicted_KNN', 'predicted_XGB',"common_value")])

table(Compared_RF_KNN_XGB$common_value)

fig1 <- plot_ly(
  my_data,
  x = ~PC1,
  y = ~PC2,
  color = Compared_RF_KNN_NB$common_value,
  colors = c('cadetblue1', 'red', 'chartreuse3', 'blueviolet'),
  type = 'scatter',
  mode = 'markers',
  symbols = c('circle', 'square', 'diamond', 'cross'),  # Specify different symbols for each color
  text = paste("Cosensus: ", Compared_RF_KNN_NB$common_value),  # Display common_value as text
  symbol = Compared_RF_KNN_NB$common_value,
...
fig2 <- plot_ly(
  ...
  symbols = c('circle', 'square', 'diamond', 'cross'),  # Specify different symbols for each color
  symbol = Compared_RF_KNN_NB$common_value,
  type = 'scatter3d',  # Specify the trace type as scatter3d for 3D plot
  mode = 'markers',
  text = paste("Cosensus: ", Compared_RF_KNN_NB$common_value),  # Display common_value as text
  hoverinfo = 'text'  # Show text on hover
)%>%
  layout(
    title = "Scatter Plot of PC1 vs PC2 vs PC3",
    xaxis = list(title = "PC1"),
    yaxis = list(title = "PC2"),
