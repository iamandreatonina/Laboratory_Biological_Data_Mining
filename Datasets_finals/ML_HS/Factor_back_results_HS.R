...
Compared_RF_KNN_NB <- Compared_RF_KNN_NB %>%
  rowwise() %>%
  mutate(common_value = {
    counts <- table(c(predicted_RF, predicted_KNN, predicted_NB))
    common_values <- names(counts[counts >= 2])
    if (length(common_values) > 0) {
      common_values[1]
    } else {
      predicted_KNN    #Chose a column  
    }
  })
...
table(Compared_RF_KNN_NB$common_value)
